...
public:
    test_mode_ipc_sink()
        ...
        , file_{filename_, std::ios_base::app}
        , pid_{getpid()}
    {
    }
...
private:
    inline static const char* const shm_name_ = "irods_test_mode_ipc_sink";
    inline static const char* const filename_ = "/var/lib/irods/log/test_mode_output.log";
    ...
    if (_write_to_stdout) {
        ...
        if (_enable_test_mode) {
            std::vector<spdlog::sink_ptr> sinks{
                std::make_shared<test_mode_ipc_sink>(),
                std::make_shared<spdlog::sinks::syslog_sink>(id, LOG_PID, LOG_LOCAL0)
            };

            log_ = std::make_shared<spdlog::logger>("test_mode", std::begin(sinks), std::end(sinks));
        }
        else {
